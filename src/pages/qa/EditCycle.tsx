  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error: cycleError } = await supabase
        .from("test_cycles")
        .update({ name, description: description || null, status: status as any })
        .eq("id", cycle.id);
        ...
        <CardContent className="p-4 sm:p-6 space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5 mb-2">
              Supports rich formatting, headings, images, and diagrams.
            </p>
            <RichTextEditor
              content={description}
              onChange={setDescription}
              placeholder="Describe the feature being tested..."
            />
          </div>
        </CardContent>
      </Card>
